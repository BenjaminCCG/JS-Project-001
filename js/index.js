window.onload=function(){
    ...
    for(let i=0;i<boxImg.length;i++){
        boxImg[i].addEventListener('mouseover',function(){
            shade[i].style.display='block'
        })
        boxImg[i].addEventListener('mouseout',function(){
        ...
        setTimeout(function(){
            ...
            [].map.call(box,(item,index)=>{
                if(index<5){
                    ...
                    heightArr[minIndex]=heightArr[minIndex]+15
                    item.style.position='absolute';
                    item.style.left=box[minIndex].offsetLeft+'px'
    ...
    window.addEventListener('scroll',function(){
        ...
        if(scroll+pageHeight>box[box.length-1].offsetTop){
            create()
            load();
        }
    })
